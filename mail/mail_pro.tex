...
\begin{titlepage}
  ...
  {\LARGE \textbf{на розробку та впровадження підсистеми електронного документообігу та обміну СЕВ ОСС} \par} \vspace{1.5cm}
...
\begin{itemize}
    ...
    \item Інтеграція з СЕВ ОСС через систему електронної взаємодії;
...
\begin{enumerate}
    ...
    \item \textbf{Етап 4: Інтеграція з СЕВ ОСС} --- Налаштування шлюзу обміну поштовими пакетами X.420. (2 місяці).
...
\begin{enumerate}
    ...
    \item \textbf{Dispatch (Відправка):} Передача через СЕВ ОСС (конверт X.420).
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{[REQ-MAIL-FUN-006]} Модуль «Документи» повинен забезпечувати інтеграцію з СЕВ ОСС для відправки та прийому документів через конверти MHS X.420.
